...
public class MoveArmAndWristSafely {
  ...
  public static void move(double extensionValue, double wristRotValue, double shoulderRotValue) {
    double extensionIn = getExtensionIn(Robot.armExtensionEncoder1.getDistanceTicks(), Robot.armExtensionEncoder2.getDistanceTicks());
    double wristRotDeg = getWristRotDegrees(Robot.armRotateEncoder1.getDistanceTicks(), Robot.armExtensionEncoder2.getDistanceTicks());
  ...
  public static boolean isMovementSafe(double extensionIn, double wristRotDeg, 
  double shoulderRotDeg, double extensionValue, double wristRotValue, 
  double shoulderRotValue) {
    ...
    if ((shoulderRotDeg > 180 && shoulderRotatingClockwise) || (shoulderRotDeg < -180 && !shoulderRotatingClockwise) || (extensionIn < 0 && !armRetracting) || (extensionIn > maxExtension && armRetracting)
        || (wristRotDeg > maxWristRot && wristRotatingClockwise) || (wristRotDeg < -maxWristRot && !wristRotatingClockwise)) {
      return false;
      ...
      if ((extensionIn > 13 || armRetracting) && (wristRotDeg < -90 || !wristRotatingClockwise) && shoulderRotDeg > 0 && shoulderRotDeg < 15) {
        return true;
      } else if ((extensionIn > 13 || armRetracting) && (wristRotDeg > 90 || wristRotatingClockwise) && shoulderRotDeg < 0 && shoulderRotDeg > -15) {
        return true;
      } else {
        return false;
      }
